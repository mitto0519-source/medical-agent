...
def _load_datasets_block(owner_email: str = "") -> str:
    ...
    parts.append("")
    parts.append("★ KNHANES 도메인 함수 (src/data/knhanes_patterns.py — 묻지 말고 그대로 호출):")
    parts.append("  - fli(df) — Fatty Liver Index (Bedogni 2006, HE_TG/HE_BMI/HE_GGT/HE_wc 자동)")
    parts.append("  - hsi(df) — Hepatic Steatosis Index (Lee 2010, HE_ALT/HE_AST/HE_BMI/sex)")
    parts.append("  - ★ fib4_index(df) — FIB-4 = (age×AST)/(plt×√ALT), Sterling 2006")
    parts.append("    fib4_stratify(s) → 'low'(<1.30) / 'intermediate'(1.30~2.67) / 'high'(>2.67)")
    parts.append("  - alcohol_category_masld(df) — MASLD-MetALD 2023 cutoff: 'no'/'moderate'/'heavy'")
    parts.append("  - cardiometabolic_risk_count(df) — MASLD 5축 충족 개수 (BMI≥23/혈당/혈압/TG/HDL)")
    parts.append("  - ★ masld_classification(df) — MASLD 2023 (Rinella) → 'MASLD'/'MetALD'/'ALD'/'not_steatosis'/'steatosis_only'")
    parts.append("  - metsx_idf_asian(df) — 대사증후군 IDF Asian")
    parts.append("  - egfr_ckd_epi_2021(df), ckd_stage(df) — CKD")
    parts.append("")
    parts.append("★ NOVA UPF 분류 (src/data/nova_classification.py — 묻지 말고 그대로 호출):")
    parts.append("  - classify_nova(food_names) — 식품명 리스트 → NOVA 1/2/3/4 (UPF는 4)")
    parts.append("  - upf_intake_share(df, intake_col='N_INTK', food_col='N_FN') — 일일 UPF 섭취 비율 (0~1)")
    parts.append("  - upf_share_by_person(df, person_col='ID') — 개인별 UPF 비율 DataFrame")
    parts.append("  - 매핑: NOVA 4 = 탄산음료/라면/과자/햄/소시지/피자/치킨/빵/초콜릿/시리얼 200+ 키워드")
    parts.append("    한국 NOVA 정밀화 (Sung 2020, Shim 2022) 키워드 매핑, 80~90% 정확")
    parts.append("")
    parts.append("★ MASLD 2023 신정의 분석 시 ultrasonography 없으면 FLI≥60 또는 HSI>36 proxy로 steatosis 양성 정의.")
    parts.append("★ ★ MASLD + FIB-4: steatosis(FLI≥60 또는 HSI>36) + FIB-4>1.30 → fibrotic MASLD (advanced fibrosis 의심) proxy.")
    parts.append("★ UPF × MASLD 분석 시 일반적 양식: upf_share_by_person(df) → tertile/quartile → masld_classification 결과와 logistic regression survey-weighted (StatBridge.fit_logit_svy).")
