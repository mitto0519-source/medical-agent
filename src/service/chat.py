...
def _load_datasets_block(owner_email: str = "") -> str:
    ...
    parts.append("  - fli(df) — Fatty Liver Index (HE_TG/HE_BMI/HE_GGT/HE_wc 자동 컬럼)")
    parts.append("  - hsi(df) — Hepatic Steatosis Index (HE_ALT/HE_AST/HE_BMI/sex)")
    ...
    parts.append("")
    parts.append("★ MASLD 2023 신정의 분석 시 ultrasonography 없으면 FLI≥60 또는 HSI>36 proxy로 steatosis 양성 정의 (코드 내장).")
    parts.append("★ NOVA(UPF) 분류는 KNHANES 식품섭취 24h 회상 데이터 (DS1_FF/DS1_NM/DS1_PR 등)를 NOVA 매핑 테이블로 변환 — 미구현이면 KYRBS F_BR(식품섭취빈도) 활용.")
