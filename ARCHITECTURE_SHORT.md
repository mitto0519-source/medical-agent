...
24. **Loop Engineering Layer (LOOP_ENGINEERING_SPEC, 2026-06-15)** — `src/loops/` registry(12 builtins wrap) + triage(4분류 inbox) + state_view(today_view) + commands(/loop /goal /triage /state /checkpoint /branch slash). `src/safety/self_bias_guard.py` (writer↔critic 다양성 강제) + `src/research/lineage.py` (provenance × Evidence Graph 조인) + `src/reliability/learning_hooks.py` (RESEARCH_STATE §6 — analysis_failure/review_score/citation_check/confidence_calibration/reproducibility_break/promotion_decision emit). peer_reviewer/evolution.gate에 hooks 실 wiring. ez_home 사이드바: Triage expander + 오늘 상태 + 인지적 항복 알림 (5턴+/골드셋 0/self_bias/confidence<0.6). models.py:_TASK_TIER에 critic_review/physician_review/stat_review=premium 추가. AGENTS.md/CLAUDE.md 규칙 11.5 Loop Catalog 박음
